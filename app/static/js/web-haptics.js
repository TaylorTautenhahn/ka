...
var WebHapticsModule = (() => {
  ...
  // tmp_js/node_modules/web-haptics/dist/index.js
  var require_index = __commonJS({
    "tmp_js/node_modules/web-haptics/dist/index.js"(exports, module) {
      var m = Object.defineProperty;
      var C = Object.getOwnPropertyDescriptor;
      var w = Object.getOwnPropertyNames;
      var M = Object.prototype.hasOwnProperty;
      var I = (s, i) => {
        for (var t in i) m(s, t, { get: i[t], enumerable: true });
      };
      var P = (s, i, t, e) => {
        if (i && typeof i == "object" || typeof i == "function") for (let n of w(i)) !M.call(s, n) && n !== t && m(s, n, { get: () => i[n], enumerable: !(e = C(i, n)) || e.enumerable });
        return s;
      };
      var k = (s) => P(m({}, "__esModule", { value: true }), s);
      var G = {};
      I(G, { WebHaptics: () => y, defaultPatterns: () => f, version: () => g });
      module.exports = k(G);
      var g = "0.0.6";
      var f = { success: { pattern: [{ duration: 30, intensity: 0.5 }, { delay: 60, duration: 40, intensity: 1 }] }, warning: { pattern: [{ duration: 40, intensity: 0.8 }, { delay: 100, duration: 40, intensity: 0.6 }] }, error: { pattern: [{ duration: 40, intensity: 0.9 }, { delay: 40, duration: 40, intensity: 0.9 }, { delay: 40, duration: 40, intensity: 0.9 }] }, light: { pattern: [{ duration: 15, intensity: 0.4 }] }, medium: { pattern: [{ duration: 25, intensity: 0.7 }] }, heavy: { pattern: [{ duration: 35, intensity: 1 }] }, soft: { pattern: [{ duration: 40, intensity: 0.5 }] }, rigid: { pattern: [{ duration: 10, intensity: 1 }] }, selection: { pattern: [{ duration: 8, intensity: 0.3 }] }, nudge: { pattern: [{ duration: 80, intensity: 0.8 }, { delay: 80, duration: 50, intensity: 0.3 }] }, buzz: { pattern: [{ duration: 1e3, intensity: 1 }] } };
      var L = 16;
      var A = 184;
      var x = 1e3;
      var b = 20;
      function S(s) {
        if (typeof s == "number") return { vibrations: [{ duration: s }] };
        if (typeof s == "string") {
          let i = f[s];
          return i ? { vibrations: i.pattern.map((t) => ({ ...t })) } : (console.warn(`[web-haptics] Unknown preset: "${s}"`), null);
        }
        if (Array.isArray(s)) {
          if (s.length === 0) return { vibrations: [] };
          ...
          return { vibrations: s.map((i) => ({ ...i })) };
        }
        return { vibrations: s.pattern.map((i) => ({ ...i })) };
      }
      function O(s, i) {
        if (i >= 1) return [s];
        if (i <= 0) return [];
        let t = Math.max(1, Math.round(b * i)), e = b - t, n = [], r = s;
        for (; r >= b; ) n.push(t), n.push(e), r -= b;
        if (r > 0) {
          let a = Math.max(1, Math.round(r * i));
          n.push(a);
          let o = r - a;
          o > 0 && n.push(o);
        }
      ...
      var y = class s {
        ...
        audioBuffer = null;
        constructor(i) {
          this.instanceId = ++j, this.debug = i?.debug ?? false, this.showSwitch = i?.showSwitch ?? false;
        }
        ...
        async trigger(i = [{ duration: 25, intensity: 0.7 }], t) {
          ...
          let r = Math.max(0, Math.min(1, t?.intensity ?? 0.5));
          for (let a of n) if (a.duration > x && (a.duration = x), !Number.isFinite(a.duration) || a.duration < 0 || a.delay !== void 0 && (!Number.isFinite(a.delay) || a.delay < 0)) {
            console.warn("[web-haptics] Invalid vibration values. Durations and delays must be finite non-negative numbers.");
            return;
          }
          if (s.isSupported && navigator.vibrate(H(n, r)), !s.isSupported || this.debug) {
            if (this.ensureDOM(), !this.hapticLabel) return;
            this.debug && await this.ensureAudio(), this.stopPattern();
            let o = (n[0]?.delay ?? 0) === 0;
            if (o && (this.hapticLabel.click(), this.debug && this.audioCtx)) {
              let d = Math.max(0, Math.min(1, n[0].intensity ?? r));
              this.playClick(d);
            }
            await this.runPattern(n, r, o);
          }
        }
        cancel() {
          this.stopPattern(), s.isSupported && navigator.vibrate(0);
        }
        destroy() {
        ...
        runPattern(i, t, e) {
          return new Promise((n) => {
            ...
            let r = [], a = 0;
            for (let l of i) {
              let c = Math.max(0, Math.min(1, l.intensity ?? t)), u = l.delay ?? 0;
              u > 0 && (a += u, r.push({ end: a, isOn: false, intensity: 0 })), a += l.duration, r.push({ end: a, isOn: true, intensity: c });
            }
            let o = a, d = 0, p = -1, v = (l) => {
              d === 0 && (d = l);
              let c = l - d;
              ...
              let u = r[0];
              for (let h of r) if (c < h.end) {
                u = h;
                break;
              }
              if (u.isOn) {
                let h = L + (1 - u.intensity) * A;
                p === -1 ? (p = l, e || (this.hapticLabel?.click(), this.debug && this.audioCtx && this.playClick(u.intensity), e = true)) : l - p >= h && (this.hapticLabel?.click(), this.debug && this.audioCtx && this.playClick(u.intensity), p = l);
              }
              this.rafId = requestAnimationFrame(v);
            };
            this.rafId = requestAnimationFrame(v);
          });
        }
...
const { WebHaptics } = WebHapticsModule; const haptics = new WebHaptics(); window.haptics = haptics;
